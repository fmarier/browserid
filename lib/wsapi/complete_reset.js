      ...
      db.completePasswordReset(req.params.token, hash, function(err, email, uid) {
        if (err) {
          ...
          wsapi.authenticateSession({session: req.session,
            ...
            if (err) return wsapi.databaseDown(res, err);
            // If the user started a password reset because of
            // a "transition_to_secondary" the type must be updated or else
            // /wsapi/address_info continues to return
            // "transition_to_secondary" as the state of the email address.
            // The user has verified the change. The transition is complete. Get
            // rid of the transition state.
