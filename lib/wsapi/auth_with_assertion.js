    ...
    db.emailType(email, function(err, type) {
      ...
      wsapi.requestToDBWriter({
        ...
        try {
          ...
          if (!r.userid) throw "response from dbwriter missing userid";
          if (!r.lastPasswordReset) throw "response from dbwriter missing lastPasswordReset";
        } catch(e) {
        ...
        wsapi.authenticateSession(
          {
            ...
            uid: r.userid,
            level: 'assertion',
            // we *just* created the account.  it might not be
            // completely stored and updated in the database.  So we
            // provide the lastPasswordReset token to the authenticateSession
            // call, rather than forcing the call to hit the database again.
            // issue #3309
