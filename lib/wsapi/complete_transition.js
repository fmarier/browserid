      ...
      db.completePasswordReset(req.params.token, null, function(err, email, uid) {
        if (err) {
          ...
          wsapi.authenticateSession({session: req.session,
            ...
            // the type must be updated or else /wsapi/address_info returns
            // "transition_to_secondary" as the state of the email address. The
            // user has verified the change. The transition is complete. Get
