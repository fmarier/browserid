...
function addEmailToAccount(userID, email, type, cb) {
  ...
  process.nextTick(function() {
    ...
    if (emails && Object.keys(emails[0]).length >= 1) {
      return cb("cannot add email address to an existing account");
    }
...
exports.checkAuth = function(userID, cb) {
  ...
  if (userID) {
    var m = jsel.match(":root > object:has(:root > .id:expr(x=?))", [ userID ], db.users);
    if (m.length === 1) {
      hash = m[0].password;
...
exports.forcefullyAddEmail = function(userID, email, type, cb) {
  process.nextTick(function() {
    sync();
    var emails = jsel.match(":has(.id:expr(x="+ ESC(userID) +")) > .emails", db.users);
    if (emails && emails.length > 0) {
